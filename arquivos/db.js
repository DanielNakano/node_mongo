...
module.exports = {
    ...
    async BuscaCompraData(data){
        const db = await connect()
        return await db.collection('compras').find({data: data}).toArray()
    ...
    async exluirVenda(data, cliente){
        const db = await connect()
        return await db.collection('vendas').deleteOne({data:data, cliente:cliente})
